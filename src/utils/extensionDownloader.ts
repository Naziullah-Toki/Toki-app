...
<body>
  ...
  function handleTabsResult(tabs) {
    ...
    if (err) {
      console.error("Error querying tabs inside handler:", err);
      showUnsupportedPage();
      return;
    }
    ...
    if (tab && tab.url) {
      ...
      // Check if we are on any google maps localized domain
      const isGoogleMaps = url.includes('google.') && (url.includes('/maps/') || url.endsWith('/maps') || url.includes('/maps?'));
  ...
  // Dual-layer fallback tab query for maximum cross-browser robustness
  try {
    chrome.tabs.query({ active: true, lastFocusedWindow: true }, (tabs) => {
      const err = chrome.runtime.lastError;
      if (err || !tabs || tabs.length === 0 || !tabs[0].url) {
        // Fallback to currentWindow
        chrome.tabs.query({ active: true, currentWindow: true }, (tabsFallback) => {
          handleTabsResult(tabsFallback);
        });
      } else {
        handleTabsResult(tabs);
      }
    });
  } catch (e) {
    console.error("Exception querying tabs:", e);
    showUnsupportedPage();
  }
  ...
  try {
    if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.local) {
      chrome.storage.local.get(['isScraping', 'scrapedLeadsCount'], (res) => {
        ...
        if (res && res.scrapedLeadsCount && scrapedCountEl) {
          scrapedCountEl.textContent = res.scrapedLeadsCount;
        }
      });
    }
  } catch (e) {
    console.error("Exception loading state:", e);
  }

  // Click listener for the startBtn
  if (startBtn) {
    startBtn.addEventListener('click', () => {
      if (!activeTabId) {
        console.warn("No active Google Maps tab identified.");
        return;
      }
      
      try {
        chrome.storage.local.get(['isScraping'], (res) => {
          const state = !(res && res.isScraping);
          chrome.storage.local.set({ isScraping: state }, () => {
            if (state) {
              // Start scraping on Google Maps active tab
              chrome.tabs.sendMessage(activeTabId, { action: "START_SCRAPING" });
            } else {
              // Stop scraping
        ...
        console.error("Exception during button action:", e);
      }
    });
  }

  // Open LeadOutfy Portal
  if (openPortalBtn) {
    openPortalBtn.addEventListener('click', () => {
      try {
        chrome.tabs.create({ url: '${appOrigin}' });
      } catch (e) {
        console.error("Exception opening desk:", e);
      }
    });
  }
});`;
...
function startScraping() {
  ...
  // Find Google Maps results scroll feed - using multiple robust fallback selectors
  let feedContainer = document.querySelector('div[role="feed"]');
  if (!feedContainer) {
    feedContainer = document.querySelector('.m6QErb[aria-label*="Results"]');
  }
  if (!feedContainer) {
    ...
    feedContainer = scrollableDivs.find(el => el.querySelector('a[href*="/maps/place/"]')) || scrollableDivs[0];
  }
  ...
  scrapeInterval = setInterval(() => {
    ...
    // 2. Select card element wrappers (supports generic child cards too)
    let cards = feedContainer.querySelectorAll('div[role="feed"] > div');
    if (cards.length === 0) {
      cards = feedContainer.querySelectorAll('.UaZ63d, .fontHeadlineSmall, .qBF1Pd');
    }
    let newLeadsFound = [];

    cards.forEach((cardOrEl) => {
      // Handle the cases where cards are selected, or we selected items directly
      const card = cardOrEl.closest('div[role="feed"] > div') || cardOrEl;
      ...
      // Extract Name with high-reliability fallback
      let name = "";
 ...
 * Listens for messages from the Chrome Extension background worker and relays them to the live React app.
 */
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === "ADD_SCRAPED_LEADS") {
    ...
    // 1. Dispatch custom event on document (highly reliable shared DOM channel)
    const event = new CustomEvent("LeadOutfySyncLeads", { detail: request.data });
    document.dispatchEvent(event);
    
    // 2. Relay via window.postMessage (highly reliable cross-world channel)
    window.postMessage({ source: "leadoutfy-portal-bridge", action: "LeadOutfySyncLeads", data: request.data }, "*");
