...
export default function App() {
  ...
  const [licenseKey, setLicenseKey] = useState('LO-7714X-ACTIVE');
  ...
  useEffect(() => {
    const handleSyncLeads = (e: Event) => {
      const customEvent = e as CustomEvent<Business[]>;
      const incomingLeads = customEvent.detail;
    ...
    window.addEventListener('LeadOutfySyncLeads', handleSyncLeads);

    return () => {
      if (scraperIntervalRef.current) clearInterval(scraperIntervalRef.current);
      window.removeEventListener('LeadOutfySyncLeads', handleSyncLeads);
    };
  }, []);
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-950 select-none overflow-hidden" id="app-root">
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        {activeTab === 'gmaps' && (
          <div className="absolute top-4 right-4 z-30 transition-all duration-300 transform scale-100 shadow-2xl">
            <LeadOutfyExtension
